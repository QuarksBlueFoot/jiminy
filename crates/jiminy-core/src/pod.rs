...
/// # Alignment
///
/// Account data on Solana is 8-byte aligned, so this is safe for any
/// type with alignment ≤ 8. For types requiring greater alignment,
/// use cursors instead.
#[inline(always)]
...
pub fn pod_write<T: Pod + FixedLayout>(data: &mut [u8], value: &T) -> Result<(), ProgramError> {
    ...
    // Safe byte-by-byte copy, no alignment requirement for destination.

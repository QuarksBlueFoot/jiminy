...
- `assert_legacy_layout!` must only validate existing `Pod + FixedLayout` types; it must not change bytes, add a header, or weaken the alignment/size contract
...
|-----------|-------|----------|
| `account_abi.rs` | 122 | Header v2, overlay, loading, extends, validation, foreign, malformed, edge cases, const offsets, split_fields, Le* types |
| `macro_ergonomics.rs` | 3 | `require*!` trailing commas, owned/borrowed key operands, legacy layout assertion bridge |
| `proptest_abi.rs` | 13 | Fuzz: roundtrip, rejection, mutation, best-effort, compatibility |
| `segments.rs` | 59 | Segmented layouts, push, swap_remove, named accessors, validation, init, compute_size |
...
| `jiminy-layouts` | 18 | Struct size assertions, legacy layout assertions, Le* roundtrips |
| Other crates | 7 | Staking, vesting, vault examples |
| **Total** | **263+** | **All passing, 0 failures when the v1 gate is green** |
...
| overlay.rs | `assert_legacy_layout!` | Compile-time trait, size, and alignment assertion for existing non-Jiminy ABIs |

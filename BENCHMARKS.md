...
| **Anchor** | Same vault logic using Anchor's account framework, Borsh serialization |
...
The `bench/anchor-vault` crate implements the same vault logic using Anchor's
account framework. This comparison isolates the framework-level overhead:

### Why the difference?

| Factor | Anchor | Jiminy |
|--------|--------|--------|
| **Serialization** | Borsh serialize on every write | Zero-copy overlay (no ser/de) |
| **Discriminator** | 8-byte SHA-256 hash, checked via generated code | 1-byte disc in 16-byte header |
| **Account parsing** | `try_deserialize()` + `try_serialize()` on every instruction | `pod_from_bytes()`: pointer cast |
| **Code generation** | Proc macros → generated boilerplate | Declarative macros → inlined checks |
| **Binary size** | ~100+ KB typical | ~27 KB |

### Compute unit breakdown (expected)

| Instruction | Pinocchio | Jiminy | Anchor | Jiminy vs Anchor |
|-------------|-----------|--------|--------|------------------|
| Deposit     | 147 CU    | 154 CU | ~1,500 CU | ~10x less |
| Withdraw    | 254 CU    | 266 CU | ~2,200 CU | ~8x less  |
| Close       | 215 CU    | 228 CU | ~1,800 CU | ~8x less  |

*Anchor CU estimates are representative of typical Anchor v0.30 programs.
Run `cargo bench` with the Anchor vault .so deployed for exact numbers.*

### Build & benchmark Anchor variant

```sh
# Build the Anchor vault for SBF
rustup run solana -- cargo build --release --target sbf-solana-solana -p bench-anchor-vault

# Copy to deploy directory
cp target/sbf-solana-solana/release/bench_anchor_vault.so target/deploy/

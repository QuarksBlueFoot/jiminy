...
describe('segments', () => {
  function makeDescriptor(offset: number, count: number, capacity: number, elementSize: number): Uint8Array {
    const buf = new Uint8Array(SEGMENT_DESCRIPTOR_SIZE);
    const view = new DataView(buf.buffer);
    view.setUint32(0, offset, true);
    view.setUint16(4, count, true);
    view.setUint16(6, capacity, true);
    view.setUint16(8, elementSize, true);
    // buf[10..12] = flags = 0 (already zero)
    return buf;
  }

  it('reads a single descriptor', () => {
    const desc = makeDescriptor(100, 3, 3, 48);
  ...
  it('reads a segment table with 2 descriptors', () => {
    const d1 = makeDescriptor(100, 2, 2, 32);
    const d2 = makeDescriptor(164, 3, 3, 48);
    ...
    expect(table[0].capacity).toBe(2);
    expect(table[1].offset).toBe(164);
    expect(table[1].capacity).toBe(3);
  });
  ...
  it('reads segment elements', () => {
    ...
    const elements = readSegmentElements(buf, { offset: 10, count: 3, capacity: 3, elementSize: 4, flags: 0 });
  ...
  it('throws if elements exceed data length', () => {
    ...
    expect(() =>
      readSegmentElements(buf, { offset: 5, count: 2, capacity: 2, elementSize: 4, flags: 0 }),
    ).toThrow('too short');
  });

  it('returns empty array for zero-count segment', () => {
    const buf = new Uint8Array(20);
    const elements = readSegmentElements(buf, { offset: 0, count: 0, capacity: 0, elementSize: 4, flags: 0 });
    expect(elements).toHaveLength(0);
  });

  it('reads descriptor at non-zero offset', () => {
    const buf = new Uint8Array(24);
    const desc = makeDescriptor(200, 5, 10, 16);
    buf.set(desc, 8);
  ...
  it('reads single-element segment', () => {
    const buf = new Uint8Array(20);
    buf[5] = 0xFF;
    const elements = readSegmentElements(buf, { offset: 5, count: 1, capacity: 1, elementSize: 1, flags: 0 });
